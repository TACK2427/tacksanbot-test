...
client.on('ready', () => {
  ...
  const statuses = [
    ...
    () => '音割れakg',  // ここで好きなテキスト1を設定
    () => '/help',  // ここで好きなテキスト2を設定
    () => '何か',  // ここで好きなテキスト2を設定
    () => 'カスタムステータス4',  // ここで好きなテキスト2を設定
  ];

  const activityTypes = [
    ActivityType.Playing,   // ゲームプレイとして表示
    ActivityType.Playing,   // ゲームプレイとして表示
    ActivityType.Listening, // 「再生」ステータスとして表示
    ActivityType.Watching, // 「再生」ステータスとして表示
    ActivityType.Competing, // 「再生」ステータスとして表示
    ActivityType.Watching, // 「再生」ステータスとして表示
  ];

  setInterval(() => {
    const currentStatus = statuses[statusIndex % statuses.length]();
    const currentActivityType = activityTypes[statusIndex % activityTypes.length];

    client.user.setActivity(currentStatus, { type: currentActivityType });
